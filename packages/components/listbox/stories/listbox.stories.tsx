...
const WithEndContentTemplate = ({
  ...
  return (
    <Listbox
      ...
      disableAnimation={disableAnimation}
      variant={variant as "flat" | "faded" | "bordered" | "solid" | "shadow" | "light" | undefined}
      onAction={(key: Key) => alert(key)}
      {...args}
    >
...
const WithBottomContentTemplate = ({color, ...args}: {color: string}) => {
  const [values, setValues] = React.useState<Selection>(new Set(["1", "3", "5"]));
...
const CustomWithClassNamesTemplate = ({color, variant, disableAnimation, ...args}: any) => {
